...
const makeTodoLi = function(newItem) {
  ...
  let newItemDone = newItem.done ? "checked" : "";

  return "<div class='update-checkbox'><input type='checkbox'"
          + newItemDone
          + "></input></div>"
...
const deleteItem = function(itemToDelete) {
  ...
  // dit zou met .closest kunnen, zoeken naar de button
...
const processResult = function(result) {
  ...
  Array.from(deleteButtons).forEach( deleteButton => deleteButton.addEventListener("click", deleteItem));
  // of 
  // [...deleteButtons].forEach( e => e.addEventListener("click", deleteItem));

  const updateBoxes = todoUl.getElementsByClassName('update-checkbox');
  Array.from(updateBoxes).forEach( updateBox => updateBox.addEventListener("change", setItemDone))
